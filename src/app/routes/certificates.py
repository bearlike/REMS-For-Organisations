...
def _create_template(metadata: EventMetadata, folder: Path) -> Path:
    ...
    draw.text((206, 1019), "This certificate is presented to", fill="#535453", font=light)
    ...
    draw.text((315, 1672), metadata.date.strftime("%d-%m-%Y"), fill="#535453", font=medium)
...
def _create_certificate_image(
    data: CertificateCSVRow, metadata: EventMetadata, template_path: Path, folder: Path, counter: int
) -> Path:
    ...
    if metadata.is_inter:
        ...
        draw = ImageDraw.Draw(img)
        name_font_size = 100 if len(data.name) <= 22 else max(30, 100 - ((len(data.name) - 22) * 7))
        name_font = ImageFont.truetype(str(FONT_MEDIUM), name_font_size)
        draw.text((206, 1019), "This certificate is presented to", fill="#535453", font=regular)
...
def generate():
    ...
    if request.method == "POST":
        ...
        with open(filepath, newline="", encoding="utf-8-sig") as csvfile:
            ...
            expected_headers = {"name", "regno", "dept", "year", "section", "email", "position", "college"}
            if reader.fieldnames is None or not expected_headers.issubset({h.strip() for h in reader.fieldnames}):
                return render_template("cert_generate.html", error="Invalid CSV headers")
            ...
            for row in reader:
                ...
                cert_link = _create_certificate_image(data, metadata, template_path, folder, counter)
        ...
        if event:
            return render_template("cert_generate.html", error="Event already exists")
        event = Event(
            event_name=metadata.event_name,
            date=metadata.date,
