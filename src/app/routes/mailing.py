            ...
            with engine.begin() as conn:
                ...
                for row in reader:
                    if len(row) < 2:
                        continue
                    conn.execute(
                        text(f"INSERT INTO {table} (name, email) VALUES (:name, :email)"),
...
def bulk_mail() -> str:
    ...
    with engine.connect() as conn:
        lists = [row[0] for row in conn.execute(text(f"SHOW TABLES FROM {cfg.mailerDB}"))]
    ...
    if request.method == "POST":
        ...
        if not form.mailing_list or not form.mail_subject:
            return render_template("mailing/bulk.html", mailing_lists=lists, cfg=cfg, error="Missing data")
        content = build_mail(
        ...
        try:
            with engine.connect() as conn:
                recipients = [r["email"] for r in conn.execute(text(f"SELECT email FROM {form.mailing_list}")).mappings()]
            for recipient in recipients:
                send_mail(recipient, form.mail_subject, content)
        except Exception:
            return render_template("mailing/bulk.html", mailing_lists=lists, cfg=cfg, error="Failed to send emails")
        log_activity(g.user, f"Sent bulk mail to list {form.mailing_list}")
        return render_template("mailing/bulk.html", mailing_lists=lists, cfg=cfg, success=True)
